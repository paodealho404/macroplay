...
def selectProfile(profile):
    ...
    # PROFILE ACTIONS
    scripts[f"/MACROS/{profile}/PROFILE ACTIONS/onBegin.txt"] = importScript(
        f"/MACROS/{profile}/PROFILE ACTIONS/onBegin.txt"
    )
    scripts[f"/MACROS/{profile}/PROFILE ACTIONS/onEnd.txt"] = importScript(
        f"/MACROS/{profile}/PROFILE ACTIONS/onEnd.txt"
    )
    scripts[f"/MACROS/{profile}/PROFILE ACTIONS/onPress.txt"] = importScript(
        f"/MACROS/{profile}/PROFILE ACTIONS/onPress.txt"
    )
    scripts[f"/MACROS/{profile}/PROFILE ACTIONS/onRelease.txt"] = importScript(
        f"/MACROS/{profile}/PROFILE ACTIONS/onRelease.txt"
    )

    # ROTARY ENCODER
    scripts[f"/MACROS/{profile}/ROTARY ENCODER/onPress.txt"] = importScript(
        f"/MACROS/{profile}/ROTARY ENCODER/onPress.txt"
    )
    scripts[f"/MACROS/{profile}/ROTARY ENCODER/onRelease.txt"] = importScript(
        f"/MACROS/{profile}/ROTARY ENCODER/onRelease.txt"
    )
    scripts[f"/MACROS/{profile}/ROTARY ENCODER/onRotateLeft.txt"] = importScript(
        f"/MACROS/{profile}/ROTARY ENCODER/onRotateLeft.txt"
    )
    scripts[f"/MACROS/{profile}/ROTARY ENCODER/onRotateRight.txt"] = importScript(
        f"/MACROS/{profile}/ROTARY ENCODER/onRotateRight.txt"
    )

    # FUNCTION SWITCHES
    for function in functionSwitches:
        scripts[f"/MACROS/{profile}/{function}/onPress.txt"] = importScript(
            f"/MACROS/{profile}/{function}/onPress.txt"
        )
        scripts[f"/MACROS/{profile}/{function}/onRelease.txt"] = importScript(
            f"/MACROS/{profile}/{function}/onRelease.txt"
        )
